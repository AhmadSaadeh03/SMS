...
export default function Student({ onLogout }) {
  ...
  const [dark, setDark]                           = useState(() => localStorage.getItem("student-theme") !== "light");
  ...
  useEffect(() => { load(); }, [load]);
  useEffect(() => { localStorage.setItem("student-theme", dark ? "dark" : "light"); }, [dark]);

  const gradeStats = useMemo(() => {
    let earned = 0, possible = 0, count = 0, exams = 0, quizzes = 0, homeworks = 0;
    for (const s of gradesData?.grades_by_subject || []) {
      for (const g of s.grades) {
        earned += g.grade_value; possible += g.max_grade; count++;
        if (g.type === "exam") exams++;
        else if (g.type === "quiz") quizzes++;
        else if (g.type === "homework") homeworks++;
      }
    }
    return { percentage: possible > 0 ? Math.round((earned / possible) * 100) : 0, earned, possible, count, exams, quizzes, homeworks };
  }, [gradesData]);

  const filteredSubjects = useMemo(() => {
    let subjects = gradesData?.grades_by_subject || [];
    ...
    return subjects;
  }, [gradesData, gradeSubjFilter, gradeTypeFilter]);

  const filteredAttendance = useMemo(() => {
    const records = attendanceData?.records || [];
    return attStatusFilter ? records.filter(r => r.status === attStatusFilter) : records;
  }, [attendanceData, attStatusFilter]);

  const attSummary     = attendanceData?.summary || { total: 0, present: 0, absent: 0, late: 0, excused: 0, percentage: 0 };
  const subjectOptions = gradesData?.grades_by_subject || [];
  ...
  const Sidebar = () => (
    <div className="flex flex-col h-full py-6 px-4"
      ...
      <div className="flex items-center gap-3 px-2 mb-8">
        ...
        <div>
          <p className={`text-sm font-bold ${t.heading}`}>Student Portal</p>
      ...
      <div className="mt-4 pt-4" style={{ borderTop: `1px solid ${t.sidebarBorder}` }}>
        <div className="flex items-center gap-3 px-2 mb-3">
          ...
          <div className="min-w-0">
            <p className={`text-sm font-medium truncate ${t.heading}`}>{user.name || "Student"}</p>
            <p className={`text-xs ${t.subheading}`}>Student</p>
          </div>
        </div>
        ...
        <main className="flex-1 p-6 overflow-auto">
          ...
          {loading ? (
            ...
            <>
              {/* Page header */}
              <div className="mb-6">
                <p className={`text-sm font-semibold ${t.label}`}>{activeLabel}</p>
                <h2 className={`text-2xl font-bold mt-1 ${t.heading}`}>
                  {profileData?.profile?.name || user.name || "My Dashboard"}
                </h2>
                <p className={`text-sm mt-1 ${t.subheading}`}>
                  {profileData?.profile?.email}{profileData?.class?.name ? ` | ${profileData.class.name}` : ""}
                </p>
              </div>
              ...
              {activeTab === "overview" && (
                <div className="space-y-6">
                  <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
                    <StatCard t={t} label="Grade Average" value={`${gradeStats.percentage}%`}
                      note={`${gradeStats.earned}/${gradeStats.possible} total marks`} color="#4f46e5" icon="📊" />
                    <StatCard t={t} label="Grade Records" value={gradeStats.count}
                      note={`${gradeStats.exams} exams · ${gradeStats.quizzes} quizzes · ${gradeStats.homeworks} homework`} color="#0891b2" icon="📋" />
                    <StatCard t={t} label="Attendance" value={`${attSummary.percentage}%`}
                      note={`${attSummary.present}/${attSummary.total} days present`} color="#16a34a" icon="📅" />
                    <StatCard t={t} label="Absent / Late" value={`${attSummary.absent}/${attSummary.late}`}
                  ...
                  <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                    <section className="rounded-2xl p-5" style={{ background: t.cardBg, border: `1px solid ${t.cardBorder}` }}>
                      <h3 className={`font-bold mb-4 ${t.heading}`}>My Subjects & Teachers</h3>
                      <div className="space-y-3">
                        {(profileData?.subjects || []).length === 0 ? (
                          ...
                          <div key={`${s.subject_id}-${s.teacher_id}`} className="rounded-xl p-4"
                            ...
                            <p className={`text-sm mt-1 ${t.subheading}`}>{s.teacher_name || "No teacher assigned"}</p>
                          </div>
                        ))}
                      </div>
                    </section>
                      ...
                      <div className="space-y-3">
                        {announcements.slice(0, 4).length === 0 ? (
                          ...
                          <div key={item.id} className="rounded-xl p-4"
                            ...
                            <p className={`text-xs mt-3 ${t.label}`}>{item.sender?.name || "Teacher"} | {item.date}</p>
                          </div>
                        ))}
                      </div>
                    </section>
              ...
              {activeTab === "grades" && (
                <div className="space-y-5">
                  ...
                  <div className="rounded-2xl p-4 flex flex-wrap gap-3 items-center justify-between"
                    ...
                    <Badge tone={gradeTone(gradeStats.percentage)}>{gradeStats.percentage}% overall average</Badge>
                  </div>

                  {filteredSubjects.length === 0 ? (
                    <div className="rounded-2xl p-10 text-center" style={{ background: t.cardBg, border: `1px solid ${t.cardBorder}` }}>
                      <p className={t.subheading}>No grades found for the selected filter.</p>
                    </div>
                  ) : filteredSubjects.map(subj => {
                    const earned   = subj.grades.reduce((s, g) => s + g.grade_value, 0);
                    const possible = subj.grades.reduce((s, g) => s + g.max_grade, 0);
              ...
              {activeTab === "attendance" && (
                <div className="space-y-5">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-5 gap-4">
                    {[
                      { label: "Total Days", value: attSummary.total,   color: "#4f46e5" },
                      ...
                      <div key={label} className="rounded-2xl p-5 text-center"
                        ...
                        <p className={`text-xs mt-1 font-semibold ${t.subheading}`}>{label}</p>
                      </div>
                    ))}
                  </div>
                  ...
                  <section className="rounded-2xl overflow-hidden" style={{ background: t.cardBg, border: `1px solid ${t.cardBorder}` }}>
                    <div className="p-5 flex flex-wrap items-center justify-between gap-4" style={{ background: t.tableHeadBg }}>
                      <div>
                        <h3 className={`font-bold ${t.heading}`}>Attendance Records</h3>
                        <p className={`text-sm mt-1 ${t.subheading}`}>Your full attendance history.</p>
                      </div>
                      <div className="flex items-center gap-3">
                      ...
                      <table className="w-full text-sm min-w-[360px]">
                        ...
                        <tbody>
                          {filteredAttendance.length === 0 ? (
                            <tr><td colSpan={2} className={`px-5 py-10 text-center ${t.subheading}`}>No attendance records found.</td></tr>
                    ...
                    {announcements.length === 0 ? (
                      ...
                      <div key={item.id} className="rounded-xl p-4"
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2">
                          <div>
                            <p className={`font-bold ${t.heading}`}>{item.title}</p>
                            <p className={`text-xs mt-1 ${t.subheading}`}>{item.sender?.name || "Teacher"} | {item.date}</p>
                          </div>
                          <Badge>teacher</Badge>
                        </div>
